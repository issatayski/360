...
(function () {
  ...
  const STILL_DPS = 12;   // телефон считается «остановленным» ниже этой угловой скорости (°/сек)
  const ALIGN_YAW = 7;    // допуск наведения по горизонтали, ° (точки в кольцо) — прощающий
  const ALIGN_PITCH = 6;  // по вертикали, °
  const HOLD_MS = 600;    // держать наведено+неподвижно до снимка
  const SETTLE_MS = 200;  // сперва замереть на это время, потом начинает копиться удержание
